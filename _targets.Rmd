...
list(
  ...
  tar_target(sim_com_output,
    sim_total %>%
      select(sim_id, scenario, connectance, stab_com, bm_total, tlvl_w_mean) %>%
      mutate(scenario = str_remove(scenario, "pred_")) %>%
      pivot_wider(
        names_from = scenario,
        names_glue = "{.value}_{scenario}",
        values_from = c(connectance, stab_com, bm_total, tlvl_w_mean)
      )
    ),
...
- Output metrics:
  ...
  - `realised_S_3`: richness in the reintroduced community, excluding the predator
  - `z_1`: ppmr in the intact community
  - `g_1`: number of trophic links of the top/removed predator in the intact
    community
  - `g_3`: number of trophic links of the top/removed predator in the reintroduced
    phase
