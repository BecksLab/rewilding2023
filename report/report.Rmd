...
i.e. `r round(nrow(failed_sim)/nrow(sim) * 100, 2)`%.
...
ti <- sim %>%
  ...
  mutate(
    ...
    l_alive_bm = map_int(bm_sp, ~length(.x[.x > 0]))
  )
check_bm_alive_match <- function(bm, alive) {
  mask <- bm >= 0 & !seq_along(bm) %in% alive
  out <- bm[mask]
...
check_bm_sp <- ti %>%
  mutate(
    bm_dead = map2(
      bm_sp, species_alive, check_bm_alive_match),
  bm_dead1 = map_dbl(bm_dead, 1)
  )
na_check_bm_sp <- check_bm_sp %>%
  filter(is.na(bm_dead))
nrow(na_check_bm_sp)
nrow(check_bm_sp)
```
...
```{r}
colnames(sim)
check_extirpation <- sim %>%
  filter(scenario == "pred_extirpated") %>%
  mutate(
    check = map2_dbl(bm_sp, i_extirpated, ~.x[.y]),
    check_init = map2_dbl(bm_init, i_extirpated, ~.x[.y]),
    check_alive = map2_lgl(species_alive, i_extirpated, ~any(.x == .y))
    )
```

```{r}
# All good
stopifnot(all(check_extirpation$check_init == 0))
check_extirpation %>%
  select(sim_id, check) %>%
  filter(check == 0)
check_extirpation %>%
...
to <- sim %>%
  filter(sim_id == 930) %>%
  arrange(scenario)
sim[sim$sim_id == 930, ]$bm_sp
sim[sim$sim_id == 930, ]$i_introduced
sim[sim$sim_id == 930, ]$richness
sim[sim$sim_id == 930, ]$richness
sim[sim$sim_id == 930, ]$bm_init
sim[sim$sim_id == 930, ]$species_alive
sim %>%
  mutate(
